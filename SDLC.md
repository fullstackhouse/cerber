...
- `merge-queue` is routing, not proof of QA.
- Only a QA reviewer sets the `qa` pipeline label. They move a queued PR from `merge-queue` to `qa` while testing, then back to `merge-queue` with `qa-approved` on pass, or to `qa-failed` on failure. Automated skills request QA with `needs-qa`; they never set `qa`.
- `blocked` and `do-not-merge` are set and cleared by humans and stop the flow wherever it is.

**This repository runs with `qaGate: false`.** `needs-qa` is therefore advisory: it flags a PR that a human should exercise before merging, and `om-merge-buddy` and `om-approve-merge-pr` surface it, but it does not by itself block the merge the way it would under a hard gate. `qa-failed`, `blocked`, and `do-not-merge` are still hard blocks. Turn the gate on in `.ai/agentic.config.json` if this repository ever wants `needs-qa` to be binding, and update this section in the same change.
...
When signals conflict, pick the higher label and say why in the label comment. A `risk-high` PR strengthens the case for `needs-qa` and deeper review even when it would otherwise look routine. In this repository the surfaces that pull a change towards `risk-high` are the ones the product's promise rests on: anything that could send data to GitHub, the trust rules, the review runner's tool grants and environment blanking, and the artifact schema that the AI and the cockpit both depend on.
...
Agents apply labels, submit reviews, and post comments **as soon as their work is done**, without waiting for CI to go green. A review submitted while checks are still running says so in its body: branch protection holds the actual merge, and the approval covers the code, not a green run. The CI outcome arrives afterwards as a follow-up comment, which also corrects the pipeline label if the result changes the verdict.

The wait for that outcome is bounded by `ci.maxWaitMinutes`, set to **20** here because this repository's CI is a single Node job (`pnpm install --frozen-lockfile`, then the three gate commands) that normally settles well inside that window. When it expires with checks still running, the agent stops waiting, runs the local validation gate as its own evidence, posts that together with the still-pending check names and an explicit statement that no further follow-up is coming, drops `ci-monitoring`, and finishes.
...
User-facing here means the cockpit under `web/` — the React review UI served by `src/server/`. Browser evidence is captured through the provider descriptor at `.ai/browsers/agent-browser.md`; `om-prepare-test-env` writes the shared environment descriptor the QA and integration-test skills consume.
...
Any non-zero exit fails the gate and blocks the PR. The implementing skills run the gate before opening a PR, and `om-check-and-commit` runs it before pushing a hand-worked branch. This list is exactly what `.github/workflows/ci.yml` runs, so a locally green gate is a real prediction of CI rather than a weaker approximation of it. The command list lives in `.ai/agentic.config.json`; when it changes, update it there, in this section, and in the CI workflow together.
